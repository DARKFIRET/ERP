...
export const getTimeSlots = async (c) => {
    try {
...
export const createBooking = async (c) => {
    ...
    try {
        ...
        if (clientRes.rows.length > 0) {
            ...
            const newClientRes = await query('INSERT INTO clients (phone, first_name) VALUES ($1, $2) RETURNING id', [phone, guestName]);
    ...
    `, [tableId, date, slotIds]);
        ...
        const bookingRes = await query('INSERT INTO bookings (table_id, client_id, guest_name, phone, booking_date, pax) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id', [tableId, clientId, guestName, phone, date, pax]);
...
export const getBookings = async (c) => {
    try {
