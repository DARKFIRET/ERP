...
export const seedDatabase = async () => {
    ...
    `);
        ...
        if (parseInt(existingUsers[0].count) === 0) {
            ...
            for (const user of users) {
                ...
                if (roleId)
                    await client.query('INSERT INTO users (username, password_hash, role_id, first_name, last_name) VALUES ($1, $2, $3, $4, $5)', [user.username, password, roleId, user.first, user.last]);
            }
        }
        // Shifts
        ...
        if (parseInt(existingTables[0].count) === 0) {
            for (let i = 1; i <= 12; i++)
                await client.query('INSERT INTO tables (number, seats, status) VALUES ($1, $2, $3)', [i, 4, 'free']);
        }
        // Clients (NEW)
        await client.query(`
    ...
    `);
        ...
        if (parseInt(existingSlots[0].count) === 0) {
            for (let h = 9; h <= 21; h++)
                await client.query('INSERT INTO time_slots (start_time) VALUES ($1)', [`${h.toString().padStart(2, '0')}:00:00`]);
        }
    ...
    `);
        try {
            await client.query('ALTER TABLE bookings ADD COLUMN IF NOT EXISTS client_id INTEGER REFERENCES clients(id)');
        }
        catch (e) { }
        try {
            await client.query('ALTER TABLE bookings ALTER COLUMN guest_name DROP NOT NULL');
        }
        catch (e) { }
        try {
        ...
        if (parseInt(existingMenu[0].count) === 0) {
            for (const item of menuItems)
                await client.query('INSERT INTO menu_items (name, price, category, image_url) VALUES ($1, $2, $3, $4)', [item.name, item.price, item.category, item.image_url]);
        }
    ...
    `);
        try {
            await client.query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS client_id INTEGER REFERENCES clients(id)');
        }
        catch (e) { }
        // Order Items
